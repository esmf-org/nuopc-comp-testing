...
- Run the configured testing applications

- Push specified model output to GitHub artifacts

- Create baseline and store it as GitHub cache. Then, checks it against the existing baseline if it is created before. The action does not store all the data generated by the output. It just calculates the MD5 hases of each file that is selected for the baseline generation (`baseline_files` argument) and store only those hases in simple ASCII file.
